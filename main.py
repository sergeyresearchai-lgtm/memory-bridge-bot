...
def handle_message(message):
    ...
    if user_text.strip() == '/debug':
        ...
        # ============================================================================
        # НАЧАЛО ТЕСТА ВЕКТОРНОЙ ПАМЯТИ (Безопасный, только для /debug)
        # ============================================================================
        debug_msg += f"\n\n--- ТЕСТ ВЕКТОРНОЙ ПАМЯТИ ---\n"
        try:
            # 1. Импортируем класс здесь, чтобы не грузить его при обычных запросах
            from vector_memory_qdrant import VectorMemoryQdrant
            
            # 2. Создаём ВРЕМЕННЫЙ экземпляр в ОТДЕЛЬНОЙ папке (безопасно!)
            temp_storage = "./qdrant_storage_TEST_DEBUG"
            temp_vm = VectorMemoryQdrant(storage_path=temp_storage)
            debug_msg += f"[✓] Модуль инициализирован (путь: {temp_storage}).\n"
            
            # 3. Сохраняем тестовую запись о факте вызова /debug
            test_text = f"Тестовый вызов команды /debug. Время: {datetime.now().isoformat()}. История: {len(memory['history'])} записей."
            temp_vm.add_memory(user_id=str(user_id), role="system", text=test_text)
            debug_msg += f"[✓] Тестовая запись сохранена.\n"
            
            # 4. Ищем все записи для этого пользователя (пустой запрос) или по ключевому слову
            vector_results = temp_vm.search_memories(user_id=str(user_id), query="тест debug", limit=3)
            debug_msg += f"[✓] Поиск выполнен. Найдено результатов: {len(vector_results)}.\n"
            
            if vector_results:
                debug_msg += "Примеры найденных записей:\n"
                for i, mem in enumerate(vector_results[:2]):  # Покажем первые 2
                    debug_msg += f"  V{i}: {mem[:80]}...\n"
            else:
                debug_msg += "Пока ничего не найдено (это нормально для первого вызова).\n"
                
            # 5. Корректно "закрываем" временный экземпляр (удаляем ссылку)
            del temp_vm
            
        except Exception as e:
            # Если ошибка — просто пишем её в отладочное сообщение, не ломая команду
            debug_msg += f"[✗] Ошибка в тесте векторной памяти:\n{type(e).__name__}: {e}\n"
        # ============================================================================
        # КОНЕЦ ТЕСТА ВЕКТОРНОЙ ПАМЯТИ
        # ============================================================================

        # Отправляем собранное отладочное сообщение пользователю
